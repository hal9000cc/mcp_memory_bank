...
После установки команда `mcp-memory-bank` становится доступна в PATH. Автор пользуется Cline, поэтому, ниже приводится настройка для Cline. Поскольку протокол стандартный, mcp-memory-bank должен работать и с другими AI агентами, но настройка может быть немного другой.
